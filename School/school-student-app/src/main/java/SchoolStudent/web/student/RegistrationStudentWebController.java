...
@Slf4j
public class RegistrationStudentWebController {

    private final RegistrationStudentService service;

    @GetMapping("register-student")
    public String showForm(ModelMap modelMap) {
        modelMap.addAttribute("request", new RegistrationStudentRequest());
        modelMap.putIfAbsent("response", new RegistrationStudentResponse());
        return "register-student";
    }

    @PostMapping("register-student")
    public String processForm(@ModelAttribute("request") RegistrationStudentRequest request,
